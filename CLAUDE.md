...
O registro das metas diárias **não é mais um check-in único por dia** (regra revertida — decisão confirmada com o usuário). Cada meta diária é registrada avulsa, a qualquer hora do dia, quantas vezes quiser: dá para marcar uma agora e outra mais tarde, e dá para corrigir um valor já enviado no mesmo dia (mesmo padrão que semanal/mensal/duração já tinham). O fechamento do dia (decisão de streak) acontece **de forma reativa, a cada registro**, a partir do estado atual do dia — não só uma vez, e não só à noite: se 3 metas já estão cumpridas, o dia conta (streak +1) na hora; se uma correção derruba o dia de 3/3 para menos, o streak e os pontos daquela meta são revertidos na hora também. Isso é uma escolha consciente: streak/pontos podem subir e descer ao longo do próprio dia (trade-off aceito em troca de poder corrigir/registrar aos poucos). Quem não registra nada num dia continua fechado como 0/3 automático pelo job agendado noturno, do jeito que já era antes.
...
- Um desafio pode ser criado com `start_date` no futuro (ex.: combinar com os amigos de começar todo mundo junto numa segunda-feira específica). Antes dessa data, ninguém pode registrar nenhuma meta (diária, semanal, mensal ou de duração) — mas já dá pra entrar no desafio e configurar as próprias metas com antecedência.
- Simetricamente, **depois do `end_date` nada mais é registrável**: a duração é fixa, então o desafio para de aceitar registro de meta (diária, semanal, mensal ou de duração) a partir do dia seguinte ao último. Histórico, pontos, streaks e ranking continuam consultáveis normalmente. Aplicado em `RecordsService.assertChallengeWindow` (etapa 23) — mesma checagem para as quatro periodicidades, todas via upsert avulso por `goalId`.
