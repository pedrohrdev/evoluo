...
**Decisão de negócio da etapa 22, removida depois**: existiu uma **meta especial** entre participantes (aplicada direto, sempre sim/não, sem prazo, puramente social) — módulo `SpecialGoals` e UI "Entre amigos". Removida a pedido do usuário: o módulo e a UI saíram do código, mas a tabela `special_goals` continua existindo no banco (não foi uma migration destrutiva) com qualquer dado antigo intacto, só não é mais lida/escrita por nada. Ver `CLAUDE.md` seção 2 "Metas especiais entre participantes (removida)".
