...
O participante não pode editar nenhum registro (diário, semanal, mensal, de duração) depois que a data à qual ele pertence já passou. Isso vale tanto para corrigir um valor lançado errado quanto para lançar um registro atrasado de um dia que ficou em branco — não existe atualização retroativa nem preenchimento tardio. O dia de HOJE é a única exceção: dentro do próprio dia, o participante pode registrar e corrigir as metas diárias quantas vezes quiser (regra revertida — ver seção 3 "Day Evaluation"), e o Day Evaluation reage a cada uma dessas mudanças. Uma vez que a data passa, porém, o resultado do dia não muda mais — nenhum caminho de recálculo alcança um dia já encerrado.
...
**Decisão de negócio nova (etapa 22), fora do modelo de metas acima**: um participante pode atribuir uma **meta especial** a outro participante do mesmo desafio — aplicada direto (sem aceite), sempre sim/não, sem prazo fixo, puramente social (nunca gera pontos, nunca afeta streak/ranking), pública dentro do desafio. Deliberadamente **não** é uma 5ª categoria de `Goal`/`GoalVersion`: não tem período, não tem versionamento de conteúdo (é criada uma vez e só transita `pending` → `completed`/`cancelled`), e não participa de nenhum cálculo de Scoring/Streak/Ranking. Por isso vive em sua própria tabela (`special_goals`, ver `docs/database-schema.md`) e seu próprio módulo (`SpecialGoals`), sem tocar em nenhum dos módulos existentes.
...
**Day Evaluation** — aplica a regra "3/3 cumpridas = dia concluído" e publica esse resultado. É a fronteira exata onde a regra do streak vive, isolada de como os pontos são calculados. Regra revisada: para o dia de HOJE, essa avaliação roda de forma reativa a cada registro/correção de meta diária (podendo concluir e "desconcluir" o dia mais de uma vez no mesmo dia, já que não existe mais check-in único); para qualquer dia que já passou, ela roda no máximo uma vez, disparada por um job agendado no fuso fixo do servidor para quem não registrou nada — e, como não existe edição retroativa nem preenchimento tardio (seção 2), nunca reabre um dia que já ficou no passado.
...
**SpecialGoals** (etapa 22) — cria e resolve (`completed`/`cancelled`) as metas especiais entre dois participantes de um mesmo desafio. Não depende de Goals nem é consumido por Scoring/Streak/Ranking — nenhum outro módulo lê `special_goals`, e o inverso também é verdade (SpecialGoals nunca lê `goals`/`goal_versions` além de validar que os dois participantes existem no desafio).
